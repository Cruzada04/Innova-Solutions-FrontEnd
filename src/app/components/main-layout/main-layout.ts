...
export class MainLayout {
  ...
  private loadRoles() {
    ...
    if (stored) {
      try { this.userRoles = JSON.parse(stored); } catch { this.userRoles = []; }
    }
  }

  hasRole(role: string): boolean {
    return this.userRoles.some(r => r === role || r === 'ROLE_' + role);
  }

  get isAdminOrTutor(): boolean {
    return this.hasRole('ADMIN') || this.hasRole('TUTOR');
  }
